...
public class ModItems {
    ...
    public static void register() {
        ...
        SOLEMN_LAMENT_WHITE = reg("solemn_lament_white",
                new SolemnLamentItem(false, WeaponStyles.apply(key("solemn_lament_white"), "solemn_lament_white")
                        .maxCount(1).rarity(Rarity.EPIC)));

        SOLEMN_SHIELD = reg("solemn_shield",
                new SolemnShieldItem(WeaponStyles.apply(key("solemn_shield"), "solemn_shield")
                        .maxCount(1).rarity(Rarity.RARE)));

        TIGER_MARK = reg("tiger_mark",
                new Item(WeaponStyles.apply(key("tiger_mark"), "tiger_mark").maxCount(64).rarity(Rarity.UNCOMMON)));

        SAVAGE_TIGER_MARK = reg("savage_tiger_mark",
                new Item(WeaponStyles.apply(key("savage_tiger_mark"), "savage_tiger_mark").maxCount(64).rarity(Rarity.RARE)));

        MIMICRY = reg("mimicry",
                new MimicryItem(WeaponStyles.apply(key("mimicry"), "mimicry").maxCount(1).rarity(Rarity.EPIC)
                        .component(DataComponentTypes.ATTRIBUTE_MODIFIERS,
                                weaponModifiers("mimicry", 12.0, -3.2))));

        DACAPO = reg("dacapo",
                new DaCapoItem(WeaponStyles.apply(key("dacapo"), "dacapo").maxCount(1).rarity(Rarity.EPIC)
                        .component(DataComponentTypes.ATTRIBUTE_MODIFIERS,
                                weaponModifiers("dacapo", 7.0, -2.4))));

        RING_BRUSH = reg("ring_brush",
                new RingBrushItem(WeaponStyles.apply(key("ring_brush"), "ring_brush").maxCount(1).rarity(Rarity.EPIC)
                        .component(DataComponentTypes.ATTRIBUTE_MODIFIERS,
        ...
        TWILIGHT = reg("twilight",
                new TwilightItem(WeaponStyles.apply(key("twilight"), "twilight").maxCount(1).rarity(Rarity.EPIC)
                        .component(DataComponentTypes.ATTRIBUTE_MODIFIERS,
                                twilightModifiers())));

        TIBIA = reg("tibia",
                new TibiaItem(WeaponStyles.apply(key("tibia"), "tibia").maxCount(1).rarity(Rarity.EPIC)
                        .component(DataComponentTypes.ATTRIBUTE_MODIFIERS,
                                reachModifiers("tibia", 10.0, -2.8, 1.0))));

        BLADESINGER = reg("bladesinger",
                new BladesingerItem(WeaponStyles.apply(key("bladesinger"), "bladesinger").maxCount(1).rarity(Rarity.EPIC)
                        .component(DataComponentTypes.ATTRIBUTE_MODIFIERS,
